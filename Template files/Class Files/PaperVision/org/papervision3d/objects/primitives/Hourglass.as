package org.papervision3d.objects.primitives {
	...
	*/
	public class Hourglass extends TriangleMesh3D
	{
		/**
		...
		*/
		...
		public var adjustCenter:Number;
		...
		* <p/>
		*/
		public function Hourglass( material:MaterialObject3D=null, radius:Number=100, height:Number=100, adjustCenter:Number=50, segmentsW:int=8, segmentsH:int=6, topRadius:Number=-1, topFace:Boolean=true, bottomFace:Boolean=true )
		{
			...
			buildCylinder(radius, height,adjustCenter, topRadius, topFace, bottomFace);
		}
	
		private function buildCylinder( fRadius:Number, fHeight:Number, adjustCenter:Number, fTopRadius:Number, fTopFace:Boolean, fBottomFace:Boolean ):void
		{
			...
			for (j=0;j<(iVer+1);j++) { // vertical
				...
				for (i=0;i<iHor;i++) { // horizontal
					...
					var fX:Number = (fZ+(fHeight/2-adjustCenter))*fRds*Math.sin(fRad2*Math.PI)/fHeight;		
                    var fY:Number = (fZ+(fHeight/2-adjustCenter))*fRds*Math.cos(fRad2*Math.PI)/fHeight;
