...
public class Tube extends TriangleMesh3D
{

private var grid:Array;
private var _radius:Number;
private var _tube:Number;
private var tubelength:Number;
private var _segmentsR:int;
private var _segmentsT:int;
private var _yUp:Boolean;


public function Tube( material:MaterialObject3D=null, radius:Number=100, tube:Number=50, tubelength:Number=100, segmentsR:int=8, segmentsT:int=6, yUp:Boolean=true)
{
...
buildTorus(_radius, _tube, tubelength, _segmentsR, _segmentsT, _yUp);
...
grid[i][j] = new Vertex3D((radius + tube*Math.cos(v))*Math.cos(u), -(radius + tube*Math.cos(v))*Math.sin(u), tubelength*Math.sin(v));
aVertice.push(grid[i][j]);
}}
}
